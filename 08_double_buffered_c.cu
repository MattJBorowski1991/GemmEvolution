...
)
{
    ...
    for (int i = lane_id; i < WMMA_M * WMMA_K; i += 32) {
        int row = i / WMMA_K;
        int col = i % WMMA_K;
        A_s[buf][warp_id][row][col] = A[(tile_row + row) * K + col];
    }
    for (int i = lane_id; i < WMMA_K * WMMA_N; i += 32) {
        int row = i / WMMA_N;
        int col = i % WMMA_N;
        B_s[buf][warp_id][row][col] = B[row * N + (tile_col + col)];
    }
    __syncthreads(); // ensure first tiles resident before fragment loads
    ...
    for (int k = WMMA_K; k < K; k += WMMA_K){
        ...
        // the below brute-force loading with nested forloops would mean 
        ...
        //         A_s[next][m][kk] = A[(tile_row + m) * K + k + kk];
        // for (int kk = 0; kk < WMMA_K; ++kk)
        //     for (int n = 0; n < WMMA_N; ++n)
        //         B_s[next][kk][n] = B[(k + kk) * N + (tile_col + n)];
        

        // Cooperative loading WITHOUT cp.async: each thread loads 256/32 = 8 elements
        // 16x16 tile = 256 elements, 32 threads → 8 elements per thread
        // To achieve true double buffering instead of this cooperative global-to-shared load use
        // cp.async for asynchronous copy (CUDA 11+, sm_80+), which is implemented below.
        // for (int i = lane_id; i < WMMA_M * WMMA_K; i += 32) {
        //     int row = i / WMMA_K;
        //     int col = i % WMMA_K;
        //     A_s[next][warp_id][row][col] = A[(tile_row + row) * K + k + col];
        // }
        // // TODO: Same for B_s: use cp.async for global-to-shared copy
        // for (int i = lane_id; i < WMMA_K * WMMA_N; i += 32) {
        //     int row = i / WMMA_N;
        //     int col = i % WMMA_N;
        //     B_s[next][warp_id][row][col] = B[(k + row) * N + (tile_col + col)];
        // }
        // __syncthreads();
    
        
        // Changed: switched from per-thread cooperative loads to cp.async (16B chunks)
        // This overlaps global→shared transfers with compute, achieving true double buffering
        // Load is now async via cp.async into per-warp shared buffers
        // Write: still via wmma::load_matrix_sync from shared into WMMA fragments
        ...
        // adding register unroll adds little vs cp.async overlap and can increase register pressure, reducing occupancy
...
int main(){
    std::cout << "===08_double_buffered_c (wmma runner)===\n";
    for (int s : TEST_SIZE){
        run_gemm_test_wmma("08_double_buffered_c", launch_double_buffered_tc_c, s, s, s,
...
// nvcc -O3 -arch=sm_86 08_double_buffered_c.cu -o 08_double_buffered_c && ./08_double_buffered_c | tee 08_double_buffered_c.txt